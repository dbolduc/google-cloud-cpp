...
opentelemetry::nostd::shared_ptr<opentelemetry::trace::Span> MakeSpanGrpc(
  ...
  auto span = GetTracer()->StartSpan(
      absl::StrCat(service.data(), "/", method.data()),
      {{OTEL_GET_TRACE_ATTR(AttrRpcSystem), "grpc"},
       {OTEL_GET_TRACE_ATTR(AttrRpcService), service},
       {OTEL_GET_TRACE_ATTR(AttrRpcMethod), method},
       {OTEL_GET_TRACE_ATTR(AttrNetTransport), "ip_tcp"}},
...
void InjectTraceContext(grpc::ClientContext& context, Options const& options) {
  ...
  // TODO(dbolduc): we could also just default this. end user could create a
  // client with one global then switch the global before making a call. seems
  // rare, tho.
  auto propagator =
      options.has<experimental::OpenTelemetryTextMapPropagatorOption>()
          ? options.get<experimental::OpenTelemetryTextMapPropagatorOption>()
