...
#ifdef GOOGLE_CLOUD_CPP_HAVE_OPENTELEMETRY
using ::google::cloud::testing_util::ClientContextTestPeer;
...
TEST(OpenTelemetry, EndSpan) {
  ...
  grpc::ClientContext context;
  ClientContextTestPeer test_context(&context);
  test_context.AddServerInitialMetadata({"header1", "value1"});
  test_context.AddServerInitialMetadata({"header2", "value2"});
  test_context.AddServerTrailingMetadata({"trailer", "value3"});
  test_context.AddServerTrailingMetadata({"trailer-bin", "\x09\x85\x75"});
...
TEST(OpenTelemetry, EndSpanFuture) {
  ...
  ClientContextTestPeer test_context(context.get());
