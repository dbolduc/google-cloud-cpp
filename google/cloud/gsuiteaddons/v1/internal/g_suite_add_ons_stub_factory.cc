...
std::shared_ptr<GSuiteAddOnsStub> CreateDefaultGSuiteAddOnsStub(
  ...
  auto service_grpc_stub =
      google::cloud::gsuiteaddons::v1::GSuiteAddOns::NewStub(channel);
  std::shared_ptr<GSuiteAddOnsStub> stub =
      std::make_shared<DefaultGSuiteAddOnsStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<GSuiteAddOnsAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<GSuiteAddOnsMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<GSuiteAddOnsLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeGSuiteAddOnsTracingStub(std::move(stub));
  }
  return stub;
}
