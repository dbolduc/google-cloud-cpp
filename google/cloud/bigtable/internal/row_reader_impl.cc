...
DefaultRowReaderImpl::DefaultRowReaderImpl(
    std::shared_ptr<bigtable::DataClient> client, std::string table_name, bigtable::RowSet row_set,
    ...
    : DefaultRowReaderImpl(std::move(client), std::string(""), std::move(table_name),
...
void DefaultRowReaderImpl::MakeRequest() {
  response_ = {};
  processed_chunks_count_ = 0;
...
bool DefaultRowReaderImpl::NextChunk() {
...
StatusOr<RowReaderImpl::OptionalRow> DefaultRowReaderImpl::Advance() {
  ...
  while (true) {
    ...
    if (rows_limit_ != bigtable::RowReader::NO_ROWS_LIMIT && rows_limit_ <= rows_count_) {
      return row;
    }
    ...
    if (!last_read_row_key_.empty()) {
      ...
      // request them again.
      row_set_ = row_set_.Intersect(bigtable::RowRange::Open(last_read_row_key_, ""));
    }
...
grpc::Status DefaultRowReaderImpl::AdvanceOrFail(OptionalRow& row) {
  row.reset();
  grpc::Status status;
  if (!stream_) {
    MakeRequest();
...
void DefaultRowReaderImpl::Cancel() {
