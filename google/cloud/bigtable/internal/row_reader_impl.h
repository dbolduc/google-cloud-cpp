...
class DefaultRowReaderImpl : public RowReaderImpl {
 public:
  DefaultRowReaderImpl(std::shared_ptr<bigtable::DataClient> client, std::string table_name,
  ...
  DefaultRowReaderImpl(std::shared_ptr<bigtable::DataClient> client, std::string app_profile_id,
            std::string table_name, bigtable::RowSet row_set, std::int64_t rows_limit,
            bigtable::Filter filter, std::unique_ptr<bigtable::RPCRetryPolicy> retry_policy,
            std::unique_ptr<bigtable::RPCBackoffPolicy> backoff_policy,
            bigtable::MetadataUpdatePolicy metadata_update_policy,
            std::unique_ptr<bigtable::internal::ReadRowsParserFactory> parser_factory);

  ~DefaultRowReaderImpl() override;

  void Cancel() override;
  //virtual StatusOr<absl::optional<bigtable::Row>> Advance() = 0;
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_BIGTABLE_INTERNAL_ROW_READER_H
