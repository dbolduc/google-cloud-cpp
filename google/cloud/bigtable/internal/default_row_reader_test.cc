...
TEST_F(DefaultRowReaderTest, RetryPolicyExhausted) {
  ...
  MockFunction<void(std::chrono::nanoseconds)> mock_sleeper;
  EXPECT_CALL(mock_sleeper, Call(std::chrono::nanoseconds(ms(10))))
      .Times(kNumRetries);
...
TEST_F(DefaultRowReaderTest, RetryInfoHeeded) {
  auto const delay = std::chrono::nanoseconds(std::chrono::minutes(5));
  auto mock = std::make_shared<MockBigtableStub>();
  EXPECT_CALL(*mock, ReadRows)
