...
#include "google/cloud/bigtable/internal/darren.h"
...
void AsyncRowSampler::StartIteration() {
  ...
  auto stream = stub_->AsyncSampleRowKeys(cq_, std::move(context), request);

  auto self = this->shared_from_this();
  AsyncStreamLoop<v2::SampleRowKeysResponse>(
      std::move(stream),
...
future<bool> AsyncRowSampler::OnRead(v2::SampleRowKeysResponse response) {
  std::cout << "OnRead() Called - " << response.row_key() << std::endl;
...
void AsyncRowSampler::OnFinish(Status const& status) {
  std::cout << "OnFinish() Called - " << status << std::endl;
  if (status.ok()) {
