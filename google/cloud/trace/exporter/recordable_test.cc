...
TEST(Recordable, TruncatableStringNotEnforcedDisplayName) {
  ...
  const std::string exactly_128_byte_long_string(
...
TEST(Recordable, TruncatableStringEnforcedDisplayName) {
  ...
  const std::string exactly_127_byte_long_string(
      "些长字符串被截断 Некоторая длинная строка, подлежащая усечению"
      "些长字符а符");
  const std::string exactly_129_byte_long_string(
      "些长字符串被截断 Некоторая длинная строка, подлежащая усечению"
      "些长字符а符д");
  const std::string exactly_130_byte_long_string(
      "些长字符串被截断 Некоторая длинная строка, подлежащая усечению"
      "些长字符а符断");
  ...
  // The last symbols of the 129 and 130 byte long strings are "д" and "断"
  // respectively. "д" is 2 bytes long and "断" is 3 bytes long. Thus, the
  // display name should be truncated to `exactly_127_byte_long_string` to
  // conform to limit of <= 128 bytes.
...
TEST(Recordable, TruncatableStringNotEnforcedAttributeString) {
  ...
  char const kExactly256byteUnicodeString[] =
      "些长字符串被截断 Некоторая длинная строка, подлежащая усечению"
      "些长字符串被截断 Некоторая длинная строка, подлежащая усечению"
      "些长字符串被截断 Некот";

  rec.SetAttribute("string_key_2",
                   common::AttributeValue(kExactly256byteUnicodeString));

  auto attr_map = rec.span().attributes().attribute_map();

  EXPECT_EQ(kLongRegularString,
            attr_map["string_key_1"].string_value().value());
  EXPECT_EQ(kExactly256byteUnicodeString,
            attr_map["string_key_2"].string_value().value());
}
...
TEST(Recordable, TruncatableStringEnforcedAttributeString) {
  ...
  const std::string exactly_255_byte_long_string(
      "些长字符串被截断 Некоторая длинная строка, подлежащая усечению"
      "些长字符а符 些长字符串被截断 Некоторая длинная строка, подлежащая "
      "усечению"
      "些长字符а符");
  const std::string exactly_257_byte_long_string(
      "些长字符串被截断 Некоторая длинная строка, подлежащая усечению"
      "些长字符а符 些长字符串被截断 Некоторая длинная строка, подлежащая "
      "усечению"
      "些长字符а符д");
  const std::string exactly_258_byte_long_string(
      "些长字符串被截断 Некоторая длинная строка, подлежащая усечению"
      "些长字符а符 些长字符串被截断 Некоторая длинная строка, подлежащая "
      "усечению"
      "些长字符а符断");
  ...
  // The last symbols of the 257 and 258 byte long strings are "д" and "断"
  // respectively. "д" is 2 bytes long and "断" is 3 bytes long. Thus, the
  // attribute string should be truncated to `exactly_255_byte_long_string` to
  // conform to limit of <= 256 bytes.
  rec.SetAttribute("string_key_1",
                   common::AttributeValue(exactly_257_byte_long_string));
  rec.SetAttribute("string_key_2",
                   common::AttributeValue(exactly_258_byte_long_string));

  auto attr_map = rec.span().attributes().attribute_map();

  EXPECT_EQ(exactly_255_byte_long_string,
            attr_map["string_key_1"].string_value().value());
  EXPECT_EQ(exactly_255_byte_long_string,
            attr_map["string_key_2"].string_value().value());
}
...
TEST(Recordable, TestSetStartTime) {
  ...
  const common::SystemTimestamp start_timestamp(start_time);
  ...
  const std::chrono::nanoseconds start_time_nanos(
      rec.span().start_time().nanos());
  const std::chrono::seconds start_time_seconds(
      rec.span().start_time().seconds());
...
TEST(Recordable, TestSetDuration) {
  ...
  const int64_t expected_unix_end_time =
      start_timestamp.time_since_epoch().count() + duration.count();
  ...
  const std::chrono::nanoseconds unix_end_time(
      std::chrono::duration_cast<std::chrono::nanoseconds>(end_time_seconds)
          .count() +
...
TEST(Recordable, Placeholder) { EXPECT_TRUE(0); }
