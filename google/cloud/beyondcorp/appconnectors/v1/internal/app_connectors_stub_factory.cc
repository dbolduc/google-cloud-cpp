...
std::shared_ptr<AppConnectorsServiceStub>
CreateDefaultAppConnectorsServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::beyondcorp::appconnectors::v1::AppConnectorsService::NewStub(channel);
  ...
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<AppConnectorsServiceAuth>(
