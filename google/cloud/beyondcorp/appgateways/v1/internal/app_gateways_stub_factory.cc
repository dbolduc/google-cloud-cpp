...
std::shared_ptr<AppGatewaysServiceStub>
CreateDefaultAppGatewaysServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::beyondcorp::appgateways::v1::AppGatewaysService::NewStub(channel);
  ...
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<AppGatewaysServiceAuth>(
