# HOWTO: using the Cloud Issuer Switch Service API C++ client in your project

This directory contains small examples showing how to use the Cloud Issuer Switch Service API C++
...
Like most Google Cloud Platform (GCP) services, Cloud Issuer Switch Service API requires that
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/paymentgateway/quickstart
   bazel build ...
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,paymentgateway]
   ```
...
1. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/paymentgateway/quickstart
...
[quickstart-link]: https://cloud.google.com/paymentgateway/docs/quickstart
